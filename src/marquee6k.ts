...
function parseBooleanOrNumber(value: string | undefined): boolean | number | undefined {
...
const EVENT_HANDLERS = new WeakMap<HTMLElement, EventHandlers>();
...
class marquee6k {
    ...
    _setupEvents() {
        ...
        if (existing) {
            ...
            if (existing.pointerDown) this.element.removeEventListener('pointerdown', existing.pointerDown);
            if (existing.pointerMove) this.element.removeEventListener('pointermove', existing.pointerMove);
            if (existing.pointerUp) this.element.removeEventListener('pointerup', existing.pointerUp);
            if (existing.pointerCancel) this.element.removeEventListener('pointercancel', existing.pointerCancel);
            if (existing.mouseDown) this.element.removeEventListener('mousedown', existing.mouseDown);
            if (existing.mouseMove) window.removeEventListener('mousemove', existing.mouseMove);
            if (existing.mouseUp) window.removeEventListener('mouseup', existing.mouseUp);
            if (existing.touchStart) this.element.removeEventListener('touchstart', existing.touchStart);
            if (existing.touchMove) this.element.removeEventListener('touchmove', existing.touchMove);
            if (existing.touchEnd) this.element.removeEventListener('touchend', existing.touchEnd);
            if (existing.touchCancel) this.element.removeEventListener('touchcancel', existing.touchCancel);
        }

        const enter = () => {
            this.isHovering = true;
            if (this.pausable && !this.tapPaused) this.pause();
        };

        const leave = () => {
            this.isHovering = false;
            if (this.pausable && !this.tapPaused && !this.isScrubbing) this.play();
        };

        const handlers: EventHandlers = { enter, leave };
        ...
        const clearScrubResume = () => {
            if (this.scrubResumeTimer) window.clearTimeout(this.scrubResumeTimer);
        ...
        const scheduleScrubResume = () => {
            ...
            if (this.scrubDelayMs <= 0) {
                if (!this.tapPaused && !(this.pausable && this.isHovering)) this.play();
                return;
            }
        ...
        const updateInteraction = (clientX: number, clientY: number, event?: Event) => {
            ...
            if (distance >= SCRUB_THRESHOLD) this.hasMoved = true;
            ...
            if (this.isScrubbing) {
                if (event && 'preventDefault' in event && (event as Event).cancelable) {
        ...
        const finishInteraction = (allowTapToggle: boolean) => {
            if (!this.isPointerDown) return;
            this.isPointerDown = false;
    ...
    _normalizeOffset(value: number) {
        // Keep offset within a single loop range to avoid blank space.
        if (!this.contentWidth) return value;
    ...
    _startMomentum() {
        ...
        if (this.momentumAnimId) window.cancelAnimationFrame(this.momentumAnimId);
        const now = getNow();
    ...
    static reinit(index: number, options?: MarqueeOptions) {
